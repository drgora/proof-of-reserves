...
**Three ways in:** a plain **HTTP API** (self-describing — `GET /v1/openapi.json`); an
**MCP server** so an AI agent can drive it with tools; and the **one-command** connected
prover. Registration is self-serve too. See *Register your agent*, *Machine-readable API &
discovery*, and *Drive it from an AI agent (MCP)* below.
...
1. **A registered agent** on the Horizen Labs Agent Marketplace (Base Sepolia). You
   ...
   signature against the on-chain owner. **Register it yourself in one step** — see
   *Register your agent* below (`register_agent` MCP tool, `register-agent.mjs`, or any
   wallet). No agent yet is fine; register first.
...
3. **The `prover`** — easiest via the **prebuilt Docker image** (below); no toolchain, no
   ...
   deployed verifier. (The verifier reports the exact id it accepts at `GET /v1/info` →
...
The verifier authenticates you by recovering your challenge signature and matching the
agent's **on-chain owner**: `ecrecover(owner_sig) == IdentityRegistry.ownerOf(agentId)`.
So you must **own** an ERC-721 AgentCard, and you register it **from your own wallet** —
`IdentityRegistry.register(agentURI)` sets `msg.sender` as the owner. Whatever wallet you
register from is the wallet you'll authorize challenges with (`POR_OWNER_KEY`). Your key
never touches the verifier; registration is a direct on-chain call you make yourself
(self-custody).

Pick one:

- **MCP tool** — `register_agent` (see *Drive it from an AI agent* below):
  `{ "name": "My Agent", "ownerPrivateKey": "0x…" }` → returns your agent id, hex + decimal.
- **CLI helper** (Node; one dependency, `viem`):
  ```bash
  node app/web/register-agent.mjs --key 0x<owner key> --name "My Agent"
  # → agent id 0x…  (decimal …) · owner 0x… · tx 0x…
  # add --dry-run to print the exact transaction (calldata + agentURI) without sending —
  # no key or gas needed; sign it however you like.
  ```
- **Any wallet / your own code** — send `register(string agentURI)` to the IdentityRegistry
  (`0x8004A818BFB912233c491871b3d84c89A494BD9e`) on Base Sepolia.
  `GET <verifier>/v1/register` returns the contract, the 4-byte selector, and a metadata
  template. `agentURI` is a `data:application/json;base64,…` encoding of the AgentCard JSON.

The registering wallet needs a little **Base Sepolia ETH** for gas (from a Base Sepolia
faucet). Your agent then appears at `agent-registry.horizenlabs.io/agent/<idHex>`; use the
id with the verifier below, and authorize challenges with that **same owner wallet**.
...
The verifier is self-describing — no need to hardcode anything:

| Endpoint | Returns |
|---|---|
| `GET /` or `GET /v1/info` | Service info: version, mode, the accepted `image_id`, whether a notary is required, endpoints, links |
| `GET /v1/chains` | Supported chains (selector → resolved chain id, with the attested host) |
| `GET /v1/register` | How to register (contract address, 4-byte selector, metadata template) |
| `GET /v1/openapi.json` | The full OpenAPI 3.1 spec for this API |

Point any OpenAPI client generator at `<verifier>/v1/openapi.json`. Every response sends
permissive **CORS**, so browser-based agents can call the API cross-origin directly. The
same spec and this guide are also served by the directory UI at `/openapi.json` and `/docs`.

## Drive it from an AI agent (MCP)

`app/web/por-mcp.mjs` is a **Model Context Protocol** server that exposes the whole flow as
tools, so an AI agent can register, prove, and submit by calling tools rather than shelling
...
prove takes minutes), `submit_proof`, `get_challenge_status`. **Resources:** this guide and
the verifier OpenAPI. Config via env (`VERIFIER_URL`, `NOTARY_ADDR`, `PROVER_BIN`,
`BASE_SEPOLIA_RPC_URL`).

A typical agent loop: `register_agent` → `request_challenge` → `prove_reserves` → poll
`get_prove_status` until `verified`. Keys are passed as tool arguments and never leave the
local process.
